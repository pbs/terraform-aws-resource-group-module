...
module "resource-group" {
    source = "github.com/pbs/terraform-resource-group-module?ref=x.y.z"
}
```
...
```bash
mkdir -p 'terraform/modules/resource-group'
gh -R 'pbs/terraform-resource-group-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/resource-group'
rm -f release.tar.gz
```
...
```bash
git remote add -f resource-group git@github.com:pbs/terraform-resource-group-module.git
git subtree add --prefix terraform/modules/resource-group resource-group main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/resource-group resource-group $REF --squash
```
